# Vendored avm-res-network-publicipaddress module for azurerm 5.x

This directory vendors `Azure/avm-res-network-publicipaddress/azurerm` from upstream repository `Azure/terraform-azurerm-avm-res-network-publicipaddress`, version `v0.2.1`.
It exists because the upstream release currently constrains `azurerm` to `>= 3.116, < 5.0`, which is incompatible with this repository's root provider constraint of `>= 5.0, < 6.0`.
