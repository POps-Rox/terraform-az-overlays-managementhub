...
resource "azurerm_monitor_diagnostic_setting" "this" {
  for_each = { for key, value in var.diagnostic_settings : key => value if length(value.log_categories) + length(value.log_groups) + length(value.metric_categories) > 0 }
...
module "public_ip_address" {
  source = "../publicipaddress-azurerm5"
  count  = var.ip_configuration != null ? (var.ip_configuration.create_public_ip == true ? 1 : 0) : var.sku == "Developer" ? 0 : 1

  location            = var.location
  name                = coalesce(try(var.ip_configuration.public_ip_address_name, null), "pip-${var.name}")
  resource_group_name = split("/", var.parent_id)[4]
  enable_telemetry    = var.enable_telemetry
  sku                 = "Standard"
  tags = try(var.ip_configuration.public_ip_tags, null) != null ? (
    try(var.ip_configuration.public_ip_merge_with_module_tags, true)) ? merge(
    var.tags, var.ip_configuration.public_ip_tags) : (
    var.ip_configuration.public_ip_tags) : (
  try(var.ip_configuration.public_ip_merge_with_module_tags, true)) ? var.tags : {}
