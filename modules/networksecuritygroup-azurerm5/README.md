# Vendored avm-res-network-networksecuritygroup module for azurerm 5.x

This directory vendors `Azure/avm-res-network-networksecuritygroup/azurerm` from upstream repository `Azure/terraform-azurerm-avm-res-network-networksecuritygroup`, version `v0.5.1`.
It exists because the upstream release currently constrains `azurerm` to `~> 4.0`, which is incompatible with this repository's root provider constraint of `>= 5.0, < 6.0`.
...
4. `main.tf`
   - `azurerm_monitor_diagnostic_setting.this dynamic block`: upstream `no metric block emitted despite metric_categories input` → vendored `enabled_metric { category = enabled_metric.value }`.
