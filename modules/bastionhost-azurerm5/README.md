# Vendored avm-res-network-bastionhost module for azurerm 5.x

This directory vendors `Azure/avm-res-network-bastionhost/azurerm` from upstream repository `Azure/terraform-azurerm-avm-res-network-bastionhost`, version `v0.9.0`.
It exists because the upstream release currently constrains `azurerm` to `~> 4.10`, which is incompatible with this repository's root provider constraint of `>= 5.0, < 6.0`.
...
2. `terraform.tf`
   - `required_providers.azurerm.version`: upstream `~> 4.10` → vendored `>= 5.0, < 6.0`.
3. `main.tf`
...
5. `main.tf`
   - `module.public_ip_address source/version`: upstream `Azure/avm-res-network-publicipaddress/azurerm version 0.2.0` → vendored `../publicipaddress-azurerm5, so the nested public IP module also satisfies azurerm 5.x`.
6. Vendored packaging
